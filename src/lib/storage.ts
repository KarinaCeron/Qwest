...
export const jobApplicationStorage = {
  getAll: async (): Promise<JobApplication[]> => {
    try {
      ...
      return data?.map(app => ({
        ...
        jobInsights: (app as any).job_insights ?? undefined,
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          job_insights: formData.jobInsights ?? null,
        } as any)
        .select()
        .single();
      ...
      return {
        ...
        jobInsights: (data as any).job_insights ?? undefined,
  ...
  update: async (id: string, updates: Partial<JobApplicationFormData>): Promise<JobApplication | null> => {
    try {
      ...
      if (updates.jobInsights !== undefined) updateData.job_insights = updates.jobInsights;
